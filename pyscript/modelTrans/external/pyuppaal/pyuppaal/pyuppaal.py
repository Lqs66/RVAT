...
class NTA:
    def __init__(self, declaration="", system="", templates=None, queries=None):
        self.declaration = declaration
        self.system = system
        self.templates = templates or []
        self.queries = queries or QueryFile()
...
</nta>""" % (cgi.escape(self.declaration), templatesxml, cgi.escape(self.system), self.queries.to_xml())
    @classmethod
        ...
        for t in self.transitions:
            ...
            for nextnode in t.nails + [t.target]:
                ...
                if curnode == t.source:
                    ...
                    label = ''
                    for a in [t.select, t.guard, t.synchronisation, t.assignment,t.comment]:
                        if a.get_value() != None:
...
class Transition:
    ...
    def __init__(self, source, target, select='', guard='', synchronisation='',
                    assignment='', action = None, controllable=True,prob = '', comment=''):
        self.source = source
        self.target = target
        ...
        self.prob = Label("probability", prob)
        self.comment = Label("comments", comment)
        self.nails = []
        self.action = action
...
class QueryFile:
    ...
    <formula>%s</formula>
    <comment>%s</comment>
    </query>""" % (cgi.escape(q), cgi.escape(comment)) for (q, comment) in self.queries])
