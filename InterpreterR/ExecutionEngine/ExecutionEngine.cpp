...
void ExecutionEngine::getMemoryAndMapGV(GlobalVariable *GV) {
...
std::string ExecutionEngine::getMangledName(const llvm::GlobalValue *GV) {
...
void ExecutionEngine::addGlobalMapping(const llvm::GlobalValue *GV, void *Addr) {
  std::lock_guard<sys::Mutex> locked(lock);
...
void ExecutionEngine::clearGlobalMappingsFromModule(Module *M) {
  ...
  for (llvm::GlobalObject &GO : M->global_objects())
    EEState.RemoveMapping(getMangledName(static_cast<const llvm::GlobalValue*>(&GO)));
}

uint64_t ExecutionEngine::updateGlobalMapping(const llvm::GlobalValue *GV,
                                              void *Addr) {
  std::lock_guard<sys::Mutex> locked(lock);
...
void *ExecutionEngine::getPointerToGlobalIfAvailable(const llvm::GlobalValue *GV) {
  std::lock_guard<sys::Mutex> locked(lock);
  return getPointerToGlobalIfAvailable(getMangledName(GV));
}

const llvm::GlobalValue *ExecutionEngine::getGlobalValueAtAddress(void *Addr) {
  std::lock_guard<sys::Mutex> locked(lock);
  ...
  if (I != EEState.getGlobalAddressReverseMap().end()) {
    ...
    for (unsigned i = 0, e = Modules.size(); i != e; ++i)
      if (llvm::GlobalValue *GV = Modules[i]->getNamedValue(Name))
        return GV;
  }
  return nullptr;
}
...
void *ExecutionEngine::getPointerToGlobal(const llvm::GlobalValue *GV) {
...
void ExecutionEngine::StoreValueToMemory(const GenericValue &Val,
  ...
  case Type::ArrayTyID: {
    ...
    const unsigned numElems = AT->getNumElements();
    const unsigned elemSize = getDataLayout().getTypeAllocSize(ElemT);
    
    for (unsigned i = 0; i < numElems; ++i) {
      StoreValueToMemory(Val.AggregateVal[i],
                        (GenericValue*)((uint8_t*)Ptr + i * elemSize),
                        ElemT);
    }
    break;
  }
  case Type::StructTyID: {
    ...
    for (unsigned i = 0; i < numElems; ++i) {
      ...
      StoreValueToMemory(Val.AggregateVal[i],
                        (GenericValue*)((uint8_t*)Ptr + offset),
                        ElemT);
    }
    break;
  }
...
void ExecutionEngine::LoadValueFromMemory(GenericValue &Result,
  ...
  case Type::DoubleTyID:
    ...
    break;
  case Type::PointerTyID:
    // llvm::outs() << "Loading pointer from memory address: " << Ptr 
    //             << " (decimal: " << (uintptr_t)Ptr << ")\n";
    // llvm::outs().flush();
    // llvm::outs() << "Value at that address is: " << *((PointerTy*)Ptr)
    //             << " (decimal: " << (uintptr_t)(*((PointerTy*)Ptr)) << ")\n";
    // llvm::outs().flush();
