...
void Interpreter::visitBranchInst(BranchInst &I) {
  ...
  // self-loop with single instruction can cause infinite loop
  if (SF.CurBB == Dest && Dest->size() == 1) {
    llvm::outs() << "WARNING: Detected infinite self-loop! BB " << *Dest 
                 << " jumps to itself with only 1 instruction.\n";
    abort();
  }
...
//
GenericValue Interpreter::executeGEPOperation(Value *Ptr, llvm::gep_type_iterator I,
...
void Interpreter::visitIntrinsicInst(IntrinsicInst &I) {
  ...
  switch (I.getIntrinsicID()) {
    ...
    case Intrinsic::fma: {
      // fmuladd(a, b, c) = a * b + c
      GenericValue Op0 = getOperandValue(I.getOperand(0), SF);
      GenericValue Op1 = getOperandValue(I.getOperand(1), SF);
      GenericValue Op2 = getOperandValue(I.getOperand(2), SF);
      GenericValue Result;
...
void Interpreter::callFunction(Function *F, ArrayRef<GenericValue> ArgVals) {
  ...
  // llvm::outs() << "Current stack depth: " << ECStack.size() << " Calling function: " << F->getName() << "\n";
  // Make a new stack frame... and fill it in.
  ECStack.emplace_back();
  ExecutionContext &StackFrame = ECStack.back();
  StackFrame.CurFunction = F;
  // llvm::outs() << "callFunction: " << F->getName() << "\n";
  // llvm::outs() << "------------------------------------------------" << "\n";
