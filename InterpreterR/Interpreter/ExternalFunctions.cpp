...
static uppllvm::Interpreter *TheInterpreter;
...
static ffi_type *ffiTypeFor(Type *Ty) {
  switch (Ty->getTypeID()) {
    ...
    case Type::IntegerTyID:
      switch (cast<IntegerType>(Ty)->getBitWidth()) {
        case 1:  return &ffi_type_uint8;  // i1 (bool) maps to uint8
        case 8:  return &ffi_type_sint8;
        case 16: return &ffi_type_sint16;
        case 32: return &ffi_type_sint32;
        case 64: return &ffi_type_sint64;
      }
      llvm::outs() << "Unhandled integer type bitwidth: "
...
static void *ffiValueFor(Type *Ty, const GenericValue &AV,
  ...
  switch (Ty->getTypeID()) {
    case Type::IntegerTyID:
      switch (cast<IntegerType>(Ty)->getBitWidth()) {
        case 1: {
          uint8_t *I1Ptr = (uint8_t *) ArgDataPtr;
          *I1Ptr = (uint8_t) AV.IntVal.getZExtValue();
          return ArgDataPtr;
        }
        case 8: {
    ...
    switch (RetTy->getTypeID()) {
      case Type::IntegerTyID:
        switch (cast<IntegerType>(RetTy)->getBitWidth()) {
          case 1:  Result.IntVal = APInt(1 , *(uint8_t *) ret.data()); break;
...
// int snprintf(char *, size_t, const char *, ...) - based on sprintf with size limit
static GenericValue lle_X_snprintf(FunctionType *FT,
                                   ArrayRef<GenericValue> Args) {
  char *OutputBuffer = (char *)GVTOP(Args[0]);
  size_t Size = (size_t)Args[1].IntVal.getZExtValue();
  ...
  // Use a temporary buffer for sprintf, then copy with size limit
  char TempBuffer[10000];
  std::vector<GenericValue> NewArgs;
  NewArgs.push_back(PTOGV((void*)&TempBuffer[0]));
  // Skip the size argument and add format string and remaining args
  NewArgs.insert(NewArgs.end(), Args.begin()+2, Args.end());
  
  // Call sprintf to format into temporary buffer
  GenericValue GV = lle_X_sprintf(FT, NewArgs);
  
  // Copy to output buffer with size limit
  size_t written = strlen(TempBuffer);
  size_t to_copy = (written < Size - 1) ? written : (Size - 1);
  memcpy(OutputBuffer, TempBuffer, to_copy);
  OutputBuffer[to_copy] = '\0';
  
  // Return the number of characters written (excluding null terminator)
  GV.IntVal = APInt(32, to_copy);
  return GV;
}
...
// fcntl(int fd, int cmd, ...) wrapper
// Handle vararg external function by providing a wrapper
static GenericValue lle_X_fcntl(FunctionType *FT,
                                ArrayRef<GenericValue> Args) {
  int fd = (int)Args[0].IntVal.getSExtValue();
  int cmd = (int)Args[1].IntVal.getSExtValue();
  
  GenericValue GV;
  // For commands that don't need the third argument
  if (cmd == F_GETFD || cmd == F_GETFL || cmd == F_GETOWN) {
    GV.IntVal = APInt(32, fcntl(fd, cmd));
    return GV;
  }
  
  // For commands that need an integer argument
  if (Args.size() >= 3) {
    long arg = (long)Args[2].IntVal.getSExtValue();
    GV.IntVal = APInt(32, fcntl(fd, cmd, arg));
  } else {
    // If called with only 2 args, call without third arg
    GV.IntVal = APInt(32, fcntl(fd, cmd));
  }
  return GV;
}

// int open(const char *path, int flags, ...) wrapper
// Handle vararg external function by providing a wrapper
static GenericValue lle_X_open(FunctionType *FT,
                               ArrayRef<GenericValue> Args) {
  const char *path = (const char *)GVTOP(Args[0]);
  int flags = (int)Args[1].IntVal.getSExtValue();
  
  GenericValue GV;
  // Check if O_CREAT or O_TMPFILE is set, which requires mode argument
  if (Args.size() >= 3) {
    mode_t mode = (mode_t)Args[2].IntVal.getZExtValue();
    GV.IntVal = APInt(32, open(path, flags, mode));
  } else {
    GV.IntVal = APInt(32, open(path, flags));
  }
  return GV;
}

// int strcmp(const char *str1, const char *str2) - wrapper
static GenericValue lle_X_strcmp(FunctionType *FT,
                                ArrayRef<GenericValue> Args) {
  const char *str1 = (const char *)GVTOP(Args[0]);
  const char *str2 = (const char *)GVTOP(Args[1]);
  
  GenericValue GV;
  // Check for null pointers
  if (str1 == nullptr || str2 == nullptr) {
    GV.IntVal = APInt(32, -1);
    return GV;
  }
  
  GV.IntVal = APInt(32, strcmp(str1, str2));
  return GV;
}

// int pthread_mutex_lock(pthread_mutex_t *mutex) - stub implementation
static GenericValue lle_X_pthread_mutex_lock(FunctionType *FT,
                                              ArrayRef<GenericValue> Args) {
  // Do nothing, just return success
  GenericValue GV;
  GV.IntVal = APInt(32, 0);  // Return 0 (success)
  return GV;
}

// int pthread_cond_wait(pthread_cond_t *cond, pthread_mutex_t *mutex) - stub implementation
static GenericValue lle_X_pthread_cond_wait(FunctionType *FT,
                                             ArrayRef<GenericValue> Args) {
  // Do nothing, just return success
  GenericValue GV;
  GV.IntVal = APInt(32, 0);  // Return 0 (success)
...
// int pthread_mutex_unlock(pthread_mutex_t *mutex) - stub implementation
static GenericValue lle_X_pthread_mutex_unlock(FunctionType *FT,
                                                ArrayRef<GenericValue> Args) {
  // Do nothing, just return success
  GenericValue GV;
  GV.IntVal = APInt(32, 0);  // Return 0 (success)
...
void Interpreter::initializeExternalFunctions() {
  ...
  Fns.FuncNames["lle_X_snprintf"]     = lle_X_snprintf;
  ...
  Fns.FuncNames["lle_X_fcntl"]        = lle_X_fcntl;
  Fns.FuncNames["lle_X_open"]         = lle_X_open;
  Fns.FuncNames["lle_X_strcmp"]         = lle_X_strcmp;

  Fns.FuncNames["lle_X_pthread_mutex_lock"] = lle_X_pthread_mutex_lock;
  Fns.FuncNames["lle_X_pthread_cond_wait"] = lle_X_pthread_cond_wait;
  Fns.FuncNames["lle_X_pthread_mutex_unlock"] = lle_X_pthread_mutex_unlock;
}
