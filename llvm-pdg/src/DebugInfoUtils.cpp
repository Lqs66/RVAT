...
/**
 * used to get the base type of a DIDerivedType
*/
...
/**
 * loop get the lowest(base) di type of a DIDerivedType
*/
DIType *pdg::dbgutils::getLowestDIType(DIType &dt)
{
  DIType *current_dt = &dt;
  if (current_dt == nullptr)
    return nullptr;
...
/**
 * remove given di type's typedef, const and volatile attributes by getting DIDerivedType's base type
*/
DIType *pdg::dbgutils::stripAttributes(DIType &dt)
{
  DIType *current_dt = &dt;
  assert(current_dt != nullptr && "cannot strip attr on null di type!");
  auto type_tag = dt.getTag(); // get the tag of current di type

  // strip typedef const and volatile attributes
  while (type_tag == dwarf::DW_TAG_typedef ||
         type_tag == dwarf::DW_TAG_const_type ||
         type_tag == dwarf::DW_TAG_volatile_type)
  {
    // current di type is DIDerivedType?
    if (DIDerivedType *didt = dyn_cast<DIDerivedType>(current_dt))
    {
      // get the base type of current derived DIType
      DIType *baseTy = didt->getBaseType();
      if (baseTy == nullptr)
        return nullptr;
      current_dt = baseTy; // update current di type
...
/** 
 * if the given di type is a member of CompositeType, get the di type's base type
*/
...
// compute di type for global value
...
/**
 * @brief get function's return value's DIType.
 * @return DIType or nullptr if no such DIType exists
*/
DIType *pdg::dbgutils::getFuncRetDIType(Function &F)
{
  //get all metadatas in this function
  SmallVector<std::pair<unsigned, MDNode *>, 4> MDs;
  F.getAllMetadata(MDs);
  //iterate all metadatas
  for (auto &MD : MDs)
  {
    MDNode *N = MD.second;
    //check if this metadata is a DISubprogram
    if (DISubprogram *subprogram = dyn_cast<DISubprogram>(N))
    {
      // get DISubroutineType(represents a subprogram type)
      auto *sub_routine = subprogram->getType();
      // get type field. such as !2 = !DISubroutineType(types: !{null, !0, !1})
      ...
      // return Ty;
      // type field's first element represents return value.
      return type_ref[0];
    }
  }
  return nullptr;
