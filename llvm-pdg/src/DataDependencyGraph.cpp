...
PreservedAnalyses pdg::DataDependencyGraph::run(Module &M, ModuleAnalysisManager &MAM){
  ...
  for (Function &F : M) {
      if (F.isDeclaration() || F.empty())
          continue;
      auto &MDResults = FAM.getResult<MemoryDependenceAnalysis>(F);
      _mem_dep_res = &MDResults;
  ...
  // outs() << "Data Dependency Graph run" << "\n";
