...
/**
 * llvm 16 不透明指针弃用 getPointerElementType
 * 
 * Ref. https://llvm.org/docs/OpaquePointers.html#:~:text=Migration%20Instructions
 ...
 * In the LLVM middle-end and backend, this is usually accomplished by inspecting the type of relevant operations instead. 
...
StructType *pdg::pdgutils::getStructTypeFromGEP(GetElementPtrInst &gep)
{
  ...
  if (StructType *struct_type = dyn_cast<StructType>(baseType)){
    return struct_type;
  }    
  return nullptr;
}
...
/**
 * @brief get the called function from a call instruction. Only handle direct calls at the moment.
*/
Function *pdg::pdgutils::getCalledFunc(CallBase &call_base)
{
  auto called_val = call_base.getCalledOperand();  //Only direct calls return not null called_val
  if (!called_val)
    return nullptr;
...
/**
 * use LLVM API users() to get all address taken instructions (load insts) that use this alloca instruction
*/
...
// format: class.cls_name.base -> class.cls_name
std::string pdg::pdgutils::stripClsTypeStrBasePostfix(std::string class_type_string)
{
  auto deli_pos = class_type_string.find(".base");
  if (deli_pos == std::string::npos)
    return class_type_string;
  return class_type_string.substr(0, deli_pos);
}
...
bool pdg::pdgutils::hasEnding(std::string const &full_str, std::string const &ending)
{
...
bool pdg::pdgutils::startWith(std::string const &full_str, std::string const &start)
{
  return (full_str.rfind(start, 0) == 0);
}
