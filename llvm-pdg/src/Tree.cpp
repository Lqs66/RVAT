...
/**
 * expand the current node, build child nodes and connect with them
*/
int pdg::TreeNode::expandNode()
{
  ...
  if (_node_di_type == nullptr)
    return 0;
  //Check if arg's debug info is modified by typedef, const and volatile. If yes, strip them to get the base type.
  DIType* dt = dbgutils::stripAttributes(*_node_di_type); 
  if (dt == nullptr)
    return 0;
  // if the di type is a member of a CompositeType, get the base type of the di type
  dt = dbgutils::stripMemberTag(*dt);
  // iterate through all the child nodes, build a tree node for each of them.

  // we only care about pointer, reference, struct and class type
  ...
  {
    DIType* pointed_obj_dt = dbgutils::getLowestDIType(*dt); // pointed_obj_dt == nullptr or pointed_obj_dt is a base type
  ...
  if (dbgutils::isStructType(*dt) || dbgutils::isClassType(*dt))
  {
    ...
    for (unsigned i = 0; i < di_node_arr.size(); i++)
    {
      ...
      //assert(field_di_type != nullptr && "field type should not be null");
...
void pdg::TreeNode::computeDerivedAddrVarsFromParent()
{
  ...
  // check if current node is a struct field, by using the parent node's di type
  if (dbgutils::isStructType(*_parent_node->getDIType()) || dbgutils::isClassType(*_parent_node->getDIType()))
  {
    is_struct_field = true;
  }

  // iterate through all the base node addr vars, and find the addr vars that are used by the current node
...
/**
 * build tree by BFS, max_tree_depth is default to 5
*/
void pdg::Tree::build(int max_tree_depth)
{
  ...
  while (!node_queue.empty()) // have more child to expand
  {
    ...
    {
      queue_size--;
      TreeNode *current_node = node_queue.front();  // get queue front
      node_queue.pop();
      _size++;  // increase tree size
