/*DeepType: https://github.com/s3team/DeepType.git*/
...
void CallGraphPass::FindCalleesWithSMLTA(CallInst *CI) {
	...
	///  
	/// We only consider the call sites with inCallID metadata.
	int ID = -1;
    ...
    if (ID == -1 || TypeMDIncallEdges.find(ID) != TypeMDIncallEdges.end()){ // If current call site has been processed by Type Metadata Analysis, return
      	return;
    }
	...
	/// 
	/// Collect DeepType analysis results. 
	/// Only Collect the results when Type Metadata Analysis don't have results.
	std::set<llvm::Function *> deeptype_candidates;
	for (auto F : FS) {
		if (F->getSection() == ".text.startup")
			continue;
		deeptype_candidates.insert(F);
	}
	
	if (deeptype_candidates.size() > 1) {
		DeepTypeIncallEdges.emplace(ID, incall::IndirectCallEdge(*CI, deeptype_candidates));
	}
